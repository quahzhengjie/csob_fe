...
// import { PartyRelationshipsManager } from './PartyRelationshipsManager';
// // With this:
...
//   // NEW STATE FOR PARTY RELATIONSHIP MANAGEMENT
//   const [isAddToCaseModalOpen, setIsAddToCaseModalOpen] = useState(false);
//   const [isRefreshingAssociations, setIsRefreshingAssociations] = useState(false);
...
//   // NEW HANDLERS FOR PARTY RELATIONSHIP MANAGEMENT
//   const handleUpdateAssociations = async (updatedAssociations: PartyAssociation[]) => {
//     setAssociations(updatedAssociations);
    
//     // Optionally refresh party data to ensure consistency
//     setIsRefreshingAssociations(true);
//     try {
...
//     } finally {
//       setIsRefreshingAssociations(false);
//     }
//   };

//   const handleAddToCaseSuccess = (caseId: string, entityName: string, roles: string[]) => {
//     // Add the new association to the local state
//     const newAssociation: PartyAssociation = {
//       caseId,
//       entityName,
//       entityType: 'Unknown', // Will be updated on refresh
//       roles
//     };
...
//             {/* ENHANCED ASSOCIATED CASES CARD WITH PARTY RELATIONSHIP MANAGEMENT */}
//             <InfoCard 
//               title="Associated Cases" 
//               count={associations.length} 
//               icon={Building}
//               priority={associations.length > 0 ? 'normal' : 'low'}
//               isLoading={isRefreshingAssociations}
//               actions={
//                 <WithPermission permission="case:update">
//                   <motion.button
//                     whileHover={{ scale: 1.05 }}
...
//                     className="flex items-center gap-1 px-3 py-1.5 rounded-md text-xs font-medium bg-blue-100 text-blue-800 hover:bg-blue-200 dark:bg-blue-900/50 dark:text-blue-300 dark:hover:bg-blue-900 transition-colors"
//                   >
//                     <Plus size={14} />
//                     Add to Case
//                   </motion.button>
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            {/* ENHANCED ASSOCIATED CASES CARD WITH ENTITY-AWARE ROLE EDITING */}
            <InfoCard 
              title="Associated Cases" 
              count={associations.length} 
              icon={Building}
            ...
            >
              {/* USING THE NEW ENTITY-AWARE ROLE EDITOR */}
              <PartyRoleEditor
                partyId={party.partyId}
                partyName={party.name}
                associations={associations}
        ...
        {/* NEW ADD TO CASE MODAL WITH ENTITY-AWARE ROLES */}
        <AddToCaseModal
          isOpen={isAddToCaseModalOpen}
          onClose={() => setIsAddToCaseModalOpen(false)}
          partyId={party.partyId}
